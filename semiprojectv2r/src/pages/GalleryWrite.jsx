...
const validateGalleryForm = (values) => {
    ...
    if(values.ginames.size ===0) {
        formErrors.contents='이미지 파일을 첨부하세요';
    }
...
const processGalleryok = async (formValues) => {
    fetch('http://localhost:8080/api/gallery/write', {
        ...
        if (response.ok) {
            ...
            alert('겔러리 글쓰기에 실패했습니다!! 다시 시도해 주세요!');
        }
    }).catch(error => {
...
const GalleryWrite = () => {
    ...
    const handleGallerySubmit = (e) => {
        ...
        if (Object.keys(formErrors).length === 0) {
            ...
            const fname = formData.getAll("ginames")[0];
            ...
            <form name="galleryfrm" id="galleryfrm" method="post"
                ...
                <div className="my-2">
                    <input type="file" name="ginames" id="ginames"
                           className={`form-control ${errors.ginames ? 'is-invalid' : ''}`}
                           multiple required/>
                ...
                {errors.recaptcha && <div className="alert alert-danger">{errors.ginames}</div>}
